...
public class NitZaKopiranjePod1 extends Thread {

    @Override
    public void run() {
        JMSContext context = connFactory.createContext();
        JMSProducer producer = context.createProducer();
        JMSConsumer consumer = context.createConsumer(queue1,"Broj = 101");
        ...
        while (true) {
            try {
                ...
                objOdg.setIntProperty("Broj", 101);
                
                producer.send(queueOdg, objOdg);

            } catch (JMSException ex) {
                Logger.getLogger(NitZaKopiranjePod1.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
